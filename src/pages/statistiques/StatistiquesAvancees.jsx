...
const StatistiquesAvancees = () => {
  const { user } = useAuth();
  const { stats, loading, error, lastUpdated, refresh } = useAdvancedStats();
  ...
  const views = [
    ...
    { label: 'Comparaisons', value: 'comparisons', icon: BarChart3 },
  ...
  const loadRealTimeData = async () => {
    try {
      const realTimeStats = await getPlaintesStats({ realTime: true });
      setRealTimeData(realTimeStats);
    } catch (err) {
      console.error('Erreur chargement données temps réel:', err);
    }
  };
  ...
  useEffect(() => {
    if (selectedView === 'realtime') {
      loadRealTimeData();
      const interval = setInterval(loadRealTimeData, 30000); // Toutes les 30 secondes
  ...
  const chartData = {
    ...
    // Temps de résolution
    resolutionTimes: {
      labels: stats.distributions.resolutionTimes.map(item => item.label),
      datasets: [{
        data: stats.distributions.resolutionTimes.map(item => item.count),
        backgroundColor: [
          colors.success, colors.info, colors.warning, 
          colors.danger, colors.purple, colors.gray
        ],
        borderWidth: 0
      }]
    },
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl">
            <p className="text-xl text-gray-700 font-semibold mb-2">Chargement des statistiques avancées...</p>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl p-8 border border-white/20">
          <div className="flex items-center justify-between mb-8">
              ...
              <div>
                ...
                <div className="flex items-center space-x-6">
                  <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg">
                    {stats.overview.complaints.total} plaintes analysées
                  </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <button className="flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
                ...
                <div className="text-center">
                  <div className="text-4xl font-bold text-yellow-600 mb-2">2.5h</div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center">
                ...
                Analyse de Performance Multi-Critères
              </h3>
              <div className="h-96">
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
                ...
                <div className="h-80">
                  <Doughnut data={chartData.resolutionTimes} options={{...chartOptions, cutout: '50%'}} />
                </div>
              </div>
